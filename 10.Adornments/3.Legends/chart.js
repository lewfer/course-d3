...
async function drawChart(container, dataFile) {
   ...
   let xScale = d3.scaleBand()
    ...
    .append("circle")
        .attr("cx", legendX)
        .attr("cy", (d,i)=>legendY + i*25) // legendY is where the first dot appears. 25 is the distance between dots
        .attr("r", 7)
        .style("fill", d=>colourScale(d.player))      

    // Add one text label for each name
    svg.selectAll("labels")
        .data(data)
        .enter()
        .append("text")
        .attr("x", legendX+15)              // add some space for the dot
        .attr("y", (d,i)=>legendY + i*25)   // legendY is where the first dot appears. 25 is the distance between dots
        .style("fill", d=>colourScale(d.player))
        .text(d=>d.player)
